...
void Foam::overlapAMIPolyPatch::calcCurRotations()
{
	const label ncp = nCopies();
	const scalar myAngle = 360.0/scalar(ncp);
	for (label copyI = 0; copyI < ncp; copyI++)
	{
	     curRotations_[copyI] = this->RodriguesRotation(rotationAxis_, copyI*myAngle);
	}
}
...
:
    ...
    surfPtr_(nullptr),
	curRotations_(pp.curRotations_)
{
    // Neighbour patch might not be valid yet so no transformation
    // calculation possible
}
